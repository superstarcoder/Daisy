...
class AgentsCog(commands.Cog):
    ...
    `hey who are you, what are you, are you a robot?`""" % message.author.mention)
                ...
                gv.chain[message.author.id] = "suggest1 " + str(message.channel.id)
            else:
                embed = embedMsg(
                    """%s you aren't an agent of the daisy squad. type `?join squad` to join :)""" % message.author.mention)
                await message.channel.send(embed=embed)

        elif (message.author.id in gv.chain) and gv.chain[message.author.id].split()[0] == "suggest1" and (
                str(message.channel.id) == gv.chain[message.author.id].split()[1]):
            agents = gv.readFile("agents.json")
            ...
            if str(message.author.id) in agents:
                ...
                if len(message.content.split(",")) < 3:
                    embed = embedMsg(
                        """%s You must have at least 3 inputs required (seperated by a comma). Type `?suggest` again to restart""" % message.author.mention)
                    await message.channel.send(embed=embed)
    ...
    `I chose to be a robot because humans are weird!, actually i do not know`""" % message.author.mention)
                    await message.channel.send(embed=embed)
                    gv.chain[message.author.id] = "suggest2 " + str(message.channel.id)
            else:
                embed = embedMsg(
                    """%s you aren't an agent of the daisy squad. type `?join squad` to join :)""" % message.author.mention)
                await message.channel.send(embed=embed)

        elif (message.author.id in gv.chain) and gv.chain[message.author.id].split()[0] == "suggest2" and (
                str(message.channel.id) == gv.chain[message.author.id].split()[1]):
            ...
            if str(message.author.id) in agents:
                ...
                if True:
                    embed = embedMsg(
                        """%s alrightt your suggestion has been made. thank you SO MUCH! your suggestion will be reviewed anytime between 5 seconds to 24 hours later, and you will recieve your credit! Also, **you can suggest more**, but make sure your data is quality and meets the requirements""" % message.author.mention)
                    await message.channel.send(embed=embed)
                    gv.chain.pop(message.author.id)
