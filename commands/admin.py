    ...
    %s unreviewed suggestions""" % (len(suggestions), num)
        ...
        #?approve [coins] [tag] [update automatically yes/no] [comment (default already exists)]
            ...
            if len(message.content.split()) >= 4:
                ...
                await message.channel.send("ERROR! required format: `?approve [coins] [tag] [update automatically yes/no] [comment (default already exists)]`")
...
###############################################################################################################################################
        ...
        elif message.author.id == gv.admin and message.content.startswith("?add") and (not (message.author.id in gv.chain)):
            bans = gv.readFile("bans.json")
            if str(message.author.id) in bans:
            ...
            if len(message.content.split()) != 3:
                await message.channel.send("you did not follow the correct format. you must do: ?add `[ID of agent who suggested] [tag]`")
    ...
    `hey who are you, what are you, are you a robot?`""" % message.author.mention)
        ...
        elif message.author.id == gv.admin and (message.author.id in gv.chain) and gv.chain[message.author.id] == "add1":
            agents = gv.readFile("agents.json")
            ...
            if str(message.author.id) in agents:
                ...
                if len(message.content.split(",")) < 3:
                    embed = embedMsg("""%s You must have at least 3 inputs required (seperated by a comma). Type `?add` again to restart""" % message.author.mention)
                    await message.channel.send(embed=embed)
    ...
    `I chose to be a robot because humans are weird!, actually i do not know`""" % message.author.mention)
        ...
        elif message.author.id == gv.admin and (message.author.id in gv.chain) and gv.chain[message.author.id] == "add2":
