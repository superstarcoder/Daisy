...
def embedShop(page):
    ...
    for listIndex in range((page-1)*lenOfPage, ((page-1)*lenOfPage)+lenOfPage):
        ...
        try:
            ...
            embed.add_field(name=emoji+" "+title, value="__"+str(cost)+" coins__"+moneyEmoji+"\n"+description+"\n\u200B", inline=False)
...
def calculateDiff(old, new, curid, oppid, bot):
    ...
    for userid in [curid, oppid]:
        ...
        for feature in ["health", "defense", "energy"]:
            print("so im in here..")
            print(new[userid][feature])
            print(old[userid][feature])
    ...
    async def on_message(self, message):
        ...
        if message.content[0] != "?" and (not (message.author.id in gv.chain)):
            ...
            if not (str(message.author.id) in agents): 
                await message.channel.send("%s you aren't an agent of the daisy squad. type `?join squad` to join :)" % message.author.mention)
                return
            if len(msg) == 2:
            ...
            if cost > agents[str(userid)]["coins"]: 
                await message.channel.send("you do not have enough money to buy this item! help daisy out more to earn more coins")
                return
            ...
            agents = gv.readFile("agents.json")
            inventory = agents[str(message.author.id)]["items"]+agents[str(message.author.id)]["houses"]+agents[str(message.author.id)]["weapons"]+agents[str(message.author.id)]["trophies"]
            shop = gv.readFile("shop.json")

            if not(item in inventory):
                if not(item in shop):
                    await message.channel.send("that item you just talked about does not exist. Please type `?shop` for list of items.")
                    return
                else:
                    await message.channel.send("you dont have that item currently. if you would like to buy it, please type `?buy "+item+"`")
                return
        ...
        #{cur: id, opp:id, id: [weapon, health, defense, energy, bet amt], id: [weapon, health, defense, energy, bet amt.]
            ...
            if message.author.id in gv.chain and gv.chain[message.author.id][0] in ["cur", "opp"]:
                await message.channel.send("You're already in a game. Please type `?forfeit` if you wanna give up and pay the price")
                return
            ...
            energy = 50
            #update fight list
            gv.fight.append({"waitingFor":int(ping), "cur":curid, curid:{"weapon":weapon, "health":100, "defense":25, "energy":50, "betAmt":betAmt}})

            await message.channel.send("%s, %s has requested to fight with you with a %s. Whoever wins gets %s coins from the other person. please type `?accept fight` or `?reject fight`. \nneither of you will win or lose money if you reject." % (msg[1],message.author.mention,weapon,betAmt))
            print(gv.fight)
            ...
            for x in gv.fight:
                if "waitingFor" in list(x) and message.author.id == x["waitingFor"]:
                    ...
                    await message.channel.send("%s has rejected %s's request to fight" % (message.author.mention, self.bot.get_user(x["cur"]).mention))
                    return
            ...
            for x in gv.fight:
                if "waitingFor" in list(x) and message.author.id == x["waitingFor"]:
                    ...
                    gv.fight[i][oppid] = {"weapon":weapon, "health":100, "defense":25, "energy":50, "betAmt":gv.fight[i][curid]["betAmt"]}
        ...
        #def checkInFight(usrid, ):
            ...
            if gv.chain[message.author.id][0] !="cur":
                await message.channel.send("You need to be in a game to use this move. Type `fight [user] [betAmt]` to fight someone.")
